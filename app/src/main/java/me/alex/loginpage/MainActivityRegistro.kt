...
class MainActivityRegistro : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        binding.brprofesores.setOnClickListener {
            val opcionprofe = Intent(this, ActivityRegistroProfes::class.java)
            startActivity(opcionprofe)
        }
            ...
            if(email.isNotEmpty() && pass.isNotEmpty() && confirmPass.isNotEmpty() ){
                if (pass == confirmPass){
                    firebaseAuth.createUserWithEmailAndPassword(email,pass).addOnCompleteListener{
                        if(it.isSuccessful){
                            val intent = Intent(this, ActivityUsuarioAlumnos::class.java)
                            startActivity(intent)
                            Toast.makeText(baseContext,"Cuenta creada correctamente!",Toast.LENGTH_SHORT).show()

                        }else{
                            Toast.makeText(baseContext,"Correo o Contraseña Inválidos!",Toast.LENGTH_SHORT).show()
                        }
